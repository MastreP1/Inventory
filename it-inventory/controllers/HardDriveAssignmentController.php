...
class HardDriveAssignmentController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        return DB::transaction(function () use ($validated) {
            ...
            if ($drive->driveStatus && !$drive->driveStatus->is_assignable) {
                return response()->json([
                    'message' => "Drive [{$drive->label}] has status [{$drive->driveStatus->name}] which is not assignable.",
                ], 422);
            }
            ...
            $inUse = DriveStatus::where('name', 'In Use')->first();
            if ($inUse) $drive->update(['drive_status_id' => $inUse->id]);
    ...
    public function end($id)
    {
        return DB::transaction(function () use ($id) {
            ...
            $available = DriveStatus::where('name', 'Available')->first();
            if ($available && $assignment->hardDrive) {
                $assignment->hardDrive->update(['drive_status_id' => $available->id]);
            }
            ...
            return response()->json(['message' => 'Drive removed from computer.']);
        });
    }
