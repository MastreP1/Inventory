...
class LookupController extends Controller
{
    ...
    public function destroySite($id)
    {
        ...
        if ($site->locations()->exists()) {
            return response()->json([
                'message' => "Cannot delete site [{$site->name}] — it still has locations assigned to it.",
            ], 422);
        }
        $site->delete();
    ...
    public function destroyDriveStatus($id)
    {
        ...
        if ($status->hardDrives()->exists()) {
            return response()->json([
                'message' => "Cannot delete état [{$status->name}] — hard drives use it.",
            ], 422);
        }
        $status->delete();
        return response()->json(['message' => 'État deleted.']);
    }
    ...
    /**
     * POST /departments
     * Create a department. Optionally accepts owner_id to immediately
     * set the first owner via the pivot table.
     */
    public function storeDepartment(Request $request)
    {
        ...
        $dept = Department::create(['name' => $validated['name']]);

        // Attach the initial owner if provided
        if (!empty($validated['owner_id'])) {
            $dept->owners()->syncWithoutDetaching([$validated['owner_id']]);
        }

        return response()->json($dept->fresh()->load('owners'), 201);
    }
